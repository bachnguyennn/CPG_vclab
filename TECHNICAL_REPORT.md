...
11. **Task order was never varied in the headline runs.** Every 20-task result uses the fixed published CPG order (aquatic_mammals first, vehicles_2 last) and every 50-task result the fixed `pair50` order. This matters asymmetrically between the mechanisms being compared: per-task LoRA re-derives each adapter from the pristine backbone and is therefore **order-invariant by construction**, whereas CPG's later tasks train on the free pool left by earlier tasks and pick through their frozen weights (Section 9.11, item 2) — so CPG's accuracy is order-dependent in principle and the magnitude of that dependence is unmeasured here. The claim that cross-task reuse is "load-bearing" would be sharpened, in either direction, by measuring it. Two specific concerns: (a) the first task receives the entire free pool and every later task competes for what remains, so results may depend on which task occupies position 1; (b) `aquatic_mammals` is both the weakest task in every run and first in the fixed order, and is the single task on which the Section 9.10 accuracy-goal gate fired — some of that +1.07 may be an artifact of its position rather than of adaptive sparsity. `--order-seed` (both trainers, deterministic shuffle, `0` = the published order) is implemented for this ablation; 2–3 shuffled orders on CPG-S@128 with a LoRA null-control run would close it cheaply.
