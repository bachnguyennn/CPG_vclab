...
- **RQ2 (efficiency retention):** Does the exact-guarantee mechanism *retain* the backbone's efficiency advantage — i.e., does porting CPG onto a backbone with ~38x fewer FLOPs cost continual-learning accuracy relative to the VGG16-CPG reference, or does parity survive the swap?
...
*Scope note on RQ2.* The FLOP gap itself (37.6x at the smallest variant) is a property of the two backbones — CascadedViT is designed for low-FLOP inference and the reference VGG16@1.5 is deliberately over-provisioned — and is **not** a contribution of this work; it is fixed before any continual learning happens. What is measured here is whether that gap *survives* the CL protocol: whether a 1.71M-parameter backbone, compacted to 60 % sparsity per task, can absorb 20–50 tasks without losing accuracy against the reference. That was not a given, and it is the claim RQ2 makes.

Headline answers: (RQ1) yes, exactly — frozen-weight drift is bit-exact 0.00e+00 at every model size (checksum-verified), and after extending the per-task store to every unmasked trainable tensor (Section 5.4), BWT is +0.000 % and old-task logits are reproduced bit-for-bit in every run; (RQ2) yes — accuracy parity with the VGG16-CPG reference is reached and exceeded at 5–32x fewer inference FLOPs (Section 9.6), so the backbone's efficiency is retained rather than spent; (RQ3) depth, not width, and ImageNet-pretrained initialization; efficiency frontier at CViT-S, accuracy frontier at CViT-XL.
...
**On cAPF — a convenience column, not a figure of merit.** cAPF prices retained accuracy in units of inference compute, which is a convenient one-number summary when comparing backbones at very different scales. It must not be read as an optimization target: accuracy is bounded above by 100 while FLOPs are unbounded below, so cAPF is maximized by shrinking the model, and a hypothetical 20 %-accuracy model at 0.001 GFLOPs would "win" at 20 000 %/GFLOP. It is also not a metric this work's method wins on — LoRA-S scores 3579 against CPG-S's 3486 (Section 9.13). **All efficiency claims in this report are therefore made on the accuracy-vs-FLOPs plane** (the Section 9.4 figure) and stated as either accuracy at matched compute or compute at matched accuracy; cAPF is retained only as a compact column. It is deliberately *not* used to answer any research question. The FLOP counts: CViT-S 0.0198 GFLOPs / 1.71 M params vs VGG16@1.5x 0.7467 GFLOPs / 75.62 M params — a 37.6x FLOP and 44x parameter gap at the smallest variant. (Reproducibility note: cAPF values are computed from the unrounded fvcore FLOP counts, e.g. ~0.01985 GFLOPs for S; recomputing from the 3-significant-digit GFLOPs shown in the tables reproduces each cAPF to within ~0.3 %.)

### 7.1 Resolution floor and statistical conventions

Accuracy differences in this report are interpreted against a measured noise floor, so that the reader does not have to guess which gaps are real.

**Run-to-run noise.** Two runs of the *identical* from-scratch configuration (CViT-S w1.0, 32px) gave 70.53 % and 70.76 % — a spread of **0.23 pts** with no intended difference between them (Section 9.13). Three-seed points on the 20-task benchmark show standard deviations of 0.25–0.50. We therefore adopt a **resolution floor of ~0.6 pts for 20-task results**: differences smaller than this are reported as *ties / unresolved* and no 20-task finding in this report rests on one. Specifically unresolved by this criterion, and labelled as such where they appear:

| Comparison | Gap | Status |
|---|:---:|---|
| CViT-M vs CViT-L, 32px (72.77 / 72.84) | 0.07 | tie |
| Width 1.5 vs 2.0, 32px (72.27 / 72.02) | 0.25 | tie — *not* a regression |
| Growth arm vs static w1.0 (71.33 ± 0.50 / 70.76) | 0.57 | unresolved |
| Growth arm, post-growth tasks (69.82 / 69.44) | 0.38 | unresolved |

**Reporting of multi-seed points.** Three seeds is enough to bound a mean, not to estimate a standard deviation precisely; a sigma estimated from n=3 is itself highly uncertain, so this report does **not** express margins in multiples of it. Multi-seed claims are stated as mean ± sample sd with an explicit 95 % t-interval (df = 2, t\* = 4.303), and comparisons between two three-seed arms use Welch's t-test:

| Point | Seeds | Mean ± sd | 95 % t-interval |
|---|---|:---:|:---:|
| CViT-S @128 | 80.31 / 79.81 / 80.09 | 80.07 ± 0.25 | [79.45, 80.69] |
| CViT-XL @128 | 82.61 / 82.00 / 82.23 | 82.28 ± 0.31 | [81.51, 83.05] |
| LoRA r=8, S @128 | 82.47 / 81.96 / 82.18 | 82.20 ± 0.26 | [81.57, 82.84] |
| CPG-S, growth at 11 | 70.89 / 71.87 / 71.23 | 71.33 ± 0.50 | [70.09, 72.57] |

**The resolution floor is regime-specific, not global.** The 0.6-pt figure above is calibrated on the 20-task benchmark, whose reported accuracy averages 20 tasks of 500 test images. The 50-task `pair50` split averages 50 tasks over the full 10,000-image test set, and is empirically far more stable: measured seed standard deviations there are **0.017 % (CPG masks: 95.46 / 95.43 / 95.43)** and **0.021 % (floor arm: 95.70 / 95.69 / 95.73)**, ~16x tighter than the 20-task arms' 0.25–0.34. Applying a blanket 0.6-pt floor to 50-task results would therefore discard real effects, and this report does not do so; 50-task comparisons are judged against their own measured spread via Welch's t. Concretely, the +0.27 floor-vs-CPG gap of Section 9.8 is inside the 20-task floor but is a highly reproducible effect in its own regime (t = 17.1, p = 0.00009, non-overlapping 95 % intervals).

| Regime | What is averaged | Measured seed sd | Floor applied |
|---|---|:---:|:---:|
| 20-task super20 | 20 tasks x 500 test images | 0.25–0.50 | ~0.6 pts |
| 50-task pair50 | 50 tasks x 200 test images (full test set) | 0.017–0.021 | own spread (Welch) |

Single-seed results are labelled as such and never carry a significance claim. Where a three-seed arm is compared against a **single-run** reference (the VGG16-CPG rows), the reference has no error bar of its own and the comparison is correspondingly weaker — this is stated at each such claim rather than left implicit.
...
| Retained accuracy | **71.33 ± 0.50 %** (70.89 / 71.87 / 71.23), 95 % t-interval [70.09, 72.57] — which contains *both* single-seed static rows (1.0: 70.76; 1.5: 72.27). **Accuracy effect unresolved** |
| Tasks 11-20 (on grown capacity) | 69.82 % vs 69.44 % in the static-1.0 run (+0.38, matched seed-1 comparison) — below the 0.6-pt resolution floor, **unresolved** |
...
**Thesis-relevant conclusion (revised):** naive uniform width-growth is architecturally incompatible with chunk-routed attention ViTs — but CPG's growing stage is not: at head/chunk granularity it is restored *exactly*, the zero-forgetting guarantee surviving a mid-stream growth event to the bit.

**This is a correctness result, and should be read as one — not as a demonstration that growth is needed.** At every horizon tested, capacity does not bind: free weights pin at 4.8 % from task ~12 of the 50-task run onward, yet retained accuracy stays flat (95.2–95.5 %) through task 50 with no growth event, picking alone carrying the last 38 tasks (Section 9.8, finding 3). Accordingly no accuracy benefit from growth should be expected here, and none is resolved: the growth arm's 95 % interval [70.09, 72.57] contains both static rows, and the post-growth-task delta (+0.38) is below the resolution floor. What the experiment establishes is that the *mechanism* is sound — the guarantee survives a mid-stream architectural change to the bit, in three independent seeds — plus a deployment property interleaved growth could never offer: growth is structured, so a task provably uses no unit added after it, and earlier tasks are served at their epoch's smaller cost. Demonstrating an accuracy *benefit* requires a capacity-bound regime (a narrower backbone, a longer stream, or the lower sparsities that `--adaptive-sparsity` selects), which this report does not construct; see Section 10, item 9. The static width/variant sweep remains the cleanest answer to the capacity-vs-accuracy question; dynamic growth answers only the mechanism question, and answers it exactly.
...
Findings: (a) the guarantee is width-independent; (b) **capacity is not the binding constraint** — 1.0 -> 1.5 buys only +1.5 pts and 1.5 -> 2.0 is a **tie** (0.25 pts, below the 0.6-pt resolution floor of Section 7.1 — this is a plateau, not a regression), so the ~6-pt gap to VGG is a backbone/training-regime effect, not a capacity limit; (c) cAPF rises as width falls, which is a property of the metric (FLOPs unbounded below, accuracy bounded above) rather than evidence for the small model — see the caveat in Section 7.
...
2. **The entire family sits far left of VGG16-CPG on the compute axis**: 6–38x fewer FLOPs and 8–44x fewer params, at 4.3–6.4 pts lower accuracy in this 32px condition (the accuracy gap closes at 128px, Section 9.6). Note this FLOP separation is inherited from the backbone choice, not produced by CPG (Section 1, scope note).
3. **Depth beats width.** M and L are a **tie** (72.77 vs 72.84, below the resolution floor) — both are wider at the same depth [1,2,3] and both land on the width-sweep plateau; XL, which adds *depth* ([1,3,4]), reaches 74.3 %, a gap of +1.5 over the M/L plateau that does clear the floor. Depth is the axis that moves accuracy; width is not.
4. **cAPF falls monotonically with size** — by construction, since FLOPs grow faster than bounded accuracy. Read on the accuracy-vs-FLOPs plane instead: CViT-S is the operating point when compute/energy is the constraint and CViT-XL when accuracy is.
...
2. **Both headline points are seed-stable, and both margins clear a 95 % interval.** Three independent seeds each: S@128 = 80.31 / 79.81 / 80.09 % (mean 80.07 ± 0.25, 95 % t-interval [79.45, 80.69]); XL@128 = 82.61 / 82.00 / 82.23 % (mean 82.28 ± 0.31, [81.51, 83.05]). Against their references: S@128 vs the VGG reproduction (78.61) gives +1.46, one-sample t = 10.1, **p = 0.010**; XL@128 vs the paper's published 81.2 gives +1.08, t = 6.07, **p = 0.026**. Both hold, with the caveat that each reference is a single run without an error bar of its own (Section 7.1). Every run measures exactly zero forgetting (BWT +0.000 %, frozen-weight drift 0.00e+00, logit drift 0.00e+00). All six runs postdate the attention-bias fix (Section 5.4); the pre-fix runs gave statistically indistinguishable accuracy (S 80.06 ± 0.18, XL 82.29 ± 0.40) with residual |BWT| <= 0.13 %.
...
6. **The comparison survives symmetric pretraining as a parity claim, not as a win (fairness control).** Giving VGG16-CPG the same advantage — ImageNet vgg16_bn init (65 tensors / 14.73M params: the complete conv stack + BN statistics, transferred via the official pipeline with the identical growth/pruning protocol; final width 1.5, same 0.7467 GFLOPs) — lifts it from 78.61 to **81.66 %** (+3.05, and +0.45 over the paper's scratch number). CViT-XL@128 (82.28 ± 0.31) is above this baseline on every seed, but the margin is only **+0.62** and does *not* clear a 95 % interval (one-sample t = 3.49, **p = 0.073**); the reference is moreover a single run with no error bar (Section 7.1). **The defensible claim against the pretrained VGG control is therefore accuracy parity at 5x fewer inference FLOPs, not a win** — which is still the result the efficiency thesis needs. CViT-S@128 (80.07 ± 0.25) trails the control by 1.6 pts at 32x fewer FLOPs. Pretraining is worth +3.05 on VGG but +7.9 on CViT, suggesting the efficient ViT converts pretrained knowledge into continual-learning accuracy more effectively under the same mechanism; with one VGG seed this remains a suggestion rather than a measurement. Resolving the +0.62 requires seeds 2–3 of the pretrained VGG arm (Section 10, item 6). Result file: `official_CPG/logs/cpg_results_imagenet.txt`.
...
### 9.7 The mechanism ladder on one frozen backbone: probe, floor, adapters, masks

Before comparing mechanisms to each other, they must be compared to *no mechanism*. With an ImageNet-pretrained backbone, 5-class tasks, and task identity known at test time, a large share of any headline number can come from pretrained features rather than from continual learning. This subsection therefore establishes a ladder on the identical frozen CViT-S@128 backbone, each rung adding exactly one thing:

- **Rung 0 — linear probe** (`probe_cvit.py`): every backbone weight frozen, per task a single fresh `nn.Linear(192, 5)` fitted on pooled frozen features. No mask, no adapter, no BN adaptation. Zero forgetting is structural. This is the pure "how much is just ImageNet features?" number.
- **Rung 1 — floor-only control** (`train_lora_cvit.py --rank 0`): adds the per-task state that *any* exact-guarantee method on this backbone must store anyway — BN affine + running statistics, SE conv biases, attention-bias tables, head — and nothing else. This isolates the adaptation mechanism from the per-task floor that Section 9.8 shows dominates the storage bill.
- **Rung 2 — per-task LoRA**: rung 1 plus a rank-r delta on every conv.
- **Rung 3 — CPG masks**: the method under study.

| Rung (all CViT-S@128, pretrained, 20 tasks) | Mechanism added | Storage/task | Retained acc | vs rung 1 |
|---|---|:---:|:---:|:---:|
| 0 — linear probe | none (frozen features + head) | ~0.004 MB | 69.51 % (1 seed) | −8.46 |
| 1 — floor-only control | per-task BN affine+stats, conv biases, attn-bias tables, head | 0.25 MB | **77.97 ± 0.34 %** (3 seeds) | — |
| 2 — LoRA r=8 | + rank-8 conv deltas | 1.17 MB | 82.20 ± 0.26 % (3 seeds) | **+4.24** (p = 0.0001) |
| 3 — CPG masks | ownership masks + piggymasks | ~0.4 MB | 80.07 ± 0.25 % (3 seeds) | **+2.10** (p = 0.0014) |

Rungs 1–3 carry three seeds each; all pairwise steps are significant under Welch's t (floor 77.62 / 78.29 / 77.99, 95 % t-interval [77.13, 78.80]).

Every rung measures exact zero forgetting (BWT +0.000 %, frozen-weight and logit drift 0.00e+00) — rungs 0 and 1 structurally, rungs 2 and 3 by the mechanisms under study.

Three findings, and the second is the uncomfortable one:

1. **The headline is not mostly pretrained features.** A pure linear probe on the frozen backbone reaches 69.51 %, more than 10 pts below both mechanisms. The ImageNet checkpoint alone does not explain the headline numbers.
2. **But most of the remaining gap is closed by the per-task floor, not by the continual-learning mechanism.** Rung 1 — per-task BatchNorm recalibration, conv biases, attention-bias tables and a head, with *no* weight-adaptation mechanism of any kind — reaches **77.97 ± 0.34 %**. That is +8.46 over the probe, a larger step than either mechanism then contributes: CPG masks add **+2.10** on top of it (Welch t = 8.70, p = 0.0014), LoRA r=8 adds **+4.24** (t = 17.39, p = 0.0001). On this benchmark the single largest lever is per-task normalization state, and both mechanisms under comparison are refinements on top of it. Any accuracy claim in this report should be read against ~78 %, not against zero.
3. **This sharpens the masks-vs-adapters verdict rather than blunting it — to a near-exact equality.** LoRA's edge over CPG (+2.13, p = 0.0005) and CPG's *entire* contribution over the mechanism-free floor (+2.10, p = 0.0014) are the same size to within 0.03 pts. **The masks buy as much over doing nothing as the adapters buy over the masks.** In mechanism terms LoRA delivers 2.0x what the masks deliver, at 2.9x the per-task storage.

Two consequences worth stating plainly. First, **the floor-only control is itself a legitimate exact-zero-forgetting continual learner** — cheaper than either mechanism (0.25 MB/task) and **statistically indistinguishable from the VGG16-CPG scratch reproduction** (77.97 ± 0.34 vs 78.61, one-sample t = −3.32, p = 0.080) at 32x fewer inference FLOPs. A reader is entitled to conclude that on this benchmark, per-task normalization state on a well-pretrained efficient backbone reproduces the reference method's accuracy without any continual-learning mechanism at all. It is not dominated by anything in this study; it is simply the cheapest point on the accuracy-storage frontier. Second, it makes Section 9.8's central finding considerably stronger: the mechanism-independent per-task floor was shown there to dominate the *storage* bill, and it turns out to dominate the *accuracy* as well.

**The ladder is benchmark-dependent, and the dependence is the finding.** Re-running rung 1 over the 50-task 2-class split (Section 9.8, finding 1b) gives 95.71 ± 0.02 % against CPG's 95.44 ± 0.02 % (3 seeds each) and LoRA r=2's 96.82 % — so on that easier benchmark the masks' +2.10 advantage measured here does not merely vanish but reverses (−0.27, p = 0.00009), while the adapters' shrinks from +4.24 to +1.11. Mechanism value is a function of how far the frozen backbone plus per-task recalibration already gets you: near a ~97 % ceiling, nothing is left for a mechanism to recover, and a mechanism that taxes every task to get there (60 % pruning) is left paying a cost with no matching benefit.

A precision note on naming: rung 1 is "mechanism-free" only in the sense that it adds no mask and no adapter. It does adapt — 29 epochs of per-task BatchNorm statistics and affine parameters is per-task normalization, a known-strong transfer baseline. It is the right floor for *this* comparison because it is exactly the state both CPG and LoRA already store per task (Section 9.8), so the delta above it isolates each mechanism's own contribution. It is not a claim that nothing is being learned.

The probe's per-task profile tracks the mechanisms' (weakest on `people` 41.2 and `aquatic_mammals` 56.8, strongest on `household_electrical_devices` 84.4 and `vehicles_2` 82.6), confirming that task difficulty is intrinsic rather than mechanism-induced (Section 9.4, finding 5). Protocol notes: the probe is fitted on un-augmented cached features, the canonical linear-probe recipe (`--train-aug` re-extracts under the augmented recipe); rung 0 remains single-seed and its −8.46 step is correspondingly the least well established number in the ladder, though it is an order of magnitude larger than any measured seed spread. Result files: `cvit_cifar/cvit_probe_20task_S_128.txt`, `cvit_cifar/cvit_floor_20task_S_128{,_seed2,_seed3}.txt`.

The remainder of this subsection is the rung-2-vs-rung-3 comparison — the on-protocol PEFT baseline the 2024-25 literature (InfLoRA, SD-LoRA, CL-LoRA) makes mandatory: freeze the ImageNet-pretrained CViT backbone and train, per task, a rank-r LoRA delta on every conv (`W + BA * alpha/r`; works for 1x1, kxk and depthwise convs), plus BatchNorm state, the SqueezeExcite conv biases, the attention-bias tables, and a fresh head. All of it is per-task state saved after the task and restored at inference, so forgetting is zero *by construction* — the exact-guarantee member of the PEFT-CL family (the published LoRA-CL methods are approximate: they minimize interference in shared trainable state). At inference the delta merges into the conv weights, so deployed FLOPs equal the plain backbone's. Same 20 tasks, loaders, epochs-equivalent recipe (29 = CPG's 25 finetune + 4 prune), optimizer, and eval instrumentation as every CPG run. Code: `cvit_cifar/lora_cvit_model.py`, `train_lora_cvit.py`.
...
1. **Per-task LoRA beats CPG masks at every matched budget on this benchmark.** At storage parity (r=2, 0.48 vs ~0.4 MB/task) LoRA leads CPG-S by +0.9 (single LoRA seed); at r=8 it leads by +2.13 with three seeds on both sides — **Welch's t = 10.3, p = 0.0005**, comfortably the most statistically secure comparison in this report — and matches CPG-XL at 6.4x fewer FLOPs. LoRA-XL reaches **84.53 %** — the best number in the entire study, +2.2 over CPG-XL and +2.9 over the ImageNet-pretrained VGG16-CPG control.
...
|---|:---:|:---:|:---:|:---:|:---:|
| **Floor only (no mechanism)** | **95.71 ± 0.02 %** (3 seeds) | **19.18 MB** | **0.249** | +0.000 % | 0.00e+00 / 0.00e+00 |
| CPG masks | 95.44 ± 0.02 % (3 seeds) | 30.76 MB | 0.459 | +0.000 % | 0.00e+00 / 0.00e+00 |
| LoRA r=2  | 96.82 % (1 seed) | 30.70 MB | 0.479 | +0.000 % | 0.00e+00 / 0.00e+00 |
| LoRA r=8  | 96.99 % (1 seed) | 65.27 MB | 1.171 | +0.000 % | 0.00e+00 / 0.00e+00 |
...
1b. **On this split the mechanism-free floor Pareto-dominates CPG masks — reproducibly, on both axes.** Running the rung-1 control (Section 9.7) over the same 50 tasks retains **95.71 ± 0.02 %** (95.70 / 95.69 / 95.73) at **19.18 MB**, against CPG masks' **95.44 ± 0.02 %** (95.46 / 95.43 / 95.43) at 30.76 MB. The floor is **+0.267 pts higher** (Welch t = 17.06, **p = 0.00009**; 95 % intervals [95.66, 95.76] and [95.40, 95.48] do not overlap) for **37.6 % less storage**, at roughly half the late-sequence slope (0.249 vs 0.459 MB/task). Storage is bit-identical across seeds on both arms, being deterministic given the recipe.

   The effect is small in absolute terms but is not noise: this split averages 50 tasks over the full 10,000-image test set, and its measured seed spread is ~0.02 pts, ~16x tighter than the 20-task arms (Section 7.1). **CPG's mechanism does not merely fail to earn its 11.6 MB here — it is slightly worse than not having it.** The most likely reading is the compaction tax: every task is pruned to 60 % sparsity whether or not it needs to be, which on near-ceiling 2-class tasks costs a little accuracy and buys capacity that is never required (finding 3). Consistent with this, restoring accuracy-goal compaction recovers ~1 pt on the 20-task split (Section 9.10).

   By contrast LoRA r=2 does buy something real — +1.11 over the floor — for its extra 11.5 MB. The mechanism comparison of Section 9.7 therefore inverts with task difficulty: on the 20-task 5-class split masks were worth +2.10 over the floor; on the 50-task 2-class split, where per-task accuracy sits near a ~97 % ceiling, they are worth slightly *less* than nothing while adapters are worth about a quarter of what they were. The natural reading is that per-task normalization state alone saturates easy tasks, and a weight-adaptation mechanism only earns its storage when the tasks are hard enough that the backbone's frozen features plus recalibration fall short. This does not weaken the storage conclusion of finding 1 — it strengthens it, by adding a cheaper arm that CPG also fails to beat. (LoRA r=2/r=8 remain single-seed; they are not part of the contested comparison, so the ±1.1 adapter margin is the one number here still resting on one run.)
2. **Why: a mechanism-independent per-task fp32 floor dominates both — on storage and, as Section 9.7 shows, on accuracy.** Any exact-zero-forgetting method on this backbone must store per-task BN statistics, attention-bias tables, and a head (~0.24 MB/task). That same floor, trained on its own with no mask and no adapter, already retains 77.97 ± 0.34 % of the 20-task benchmark (Section 9.7, rung 1) and 95.71 ± 0.02 % of this one — more than CPG masks manage (finding 1b) — so it is not merely a storage overhead the mechanisms carry, it is where most, and here all, of the accuracy comes from. CPG's binarized piggymask adds ~0.21 MB/task; LoRA r=2's adapters add ~0.24 MB/task. The *mechanism* is a minority share of the per-task bill on both sides, so the storage lines run near-parallel — the crossover the mask design promises is neutralized by overhead the masks cannot remove. Design implication: to beat adapters on storage, shrink the fp32 floor (shared or quantized per-task BN/head state), not the mask.
...
Result (20-task S@128 pretrained, **seed 1 only**): **81.14 %** retained vs the fixed-sparsity baseline's 80.07 ± 0.25 % (3 seeds) — **+1.07 pts**, with BWT +0.000 % and drift 0.00e+00. This is a single run against a three-seed mean and carries **no significance claim**; the +1.07 is well outside the baseline's seed range (79.81–80.31), which makes it promising, not established. The gate fired on exactly one task: aquatic_mammals (historically the weakest first task), where the 0.6-sparsity prune cost 18.3 train-accuracy points and the goal kept it at 0.2; all 19 remaining tasks met the goal at 0.6. Free capacity still ended at 2.4 %, so the lower first-task sparsity did not exhaust the pool at 20 tasks (a longer stream would need the growth mechanism of Section 8). The gap to LoRA r=8 (82.20 ± 0.26) narrows from 2.13 to 1.06 pts, directly confirming the pruning-tax attribution — half of LoRA's edge was the fixed compaction schedule, not the mask mechanism itself. Single seed so far; needs seeds 2–3 before headline use.

### 9.11 Task order and the first-task penalty

Every headline run in this report uses one fixed task order, so CPG's order dependence had never been measured — a gap that matters because CPG's later tasks train on the free pool left by earlier ones and *pick* through their frozen weights, while per-task LoRA re-derives every adapter from the pristine backbone and is order-invariant by construction. `--order-seed` (deterministic shuffle of the same task set, `0` = the published order) was added to all three trainers and the ablation run at the headline S@128 operating point, torch seed held at 1 so only the order varies (`run_order_ablation.sh`).

| Arm | order 0 (published) | order-seed 2 | order-seed 3 |
|---|:---:|:---:|:---:|
| CPG masks, S@128 | 80.07 ± 0.25 % (3 seeds: 80.31 / 79.81 / 80.09) | **79.65 %** | **80.65 %** |
| LoRA r=8, S@128 | 82.20 ± 0.26 % (3 seeds) | **81.93 %** | — |

**The LoRA null control behaves as it must.** Shuffling the order moves LoRA by −0.27 (1.1 sd of its own seed spread) — the deviation expected from a different RNG stream, not from structural order dependence. No state leaks across tasks in the arm the whole Section 9.7 comparison rests on.

**CPG's spread across orders exceeds its spread across seeds**, 1.00 pt (79.65–80.65) against 0.50 pt at fixed order. With one run per shuffled order, order effect and run-to-run noise cannot be separated cleanly — a single new order differs from the baseline by (order effect + noise) — so this alone is suggestive rather than conclusive. What makes it interpretable is the per-task breakdown.

**A first-task penalty explains both the spread and its direction.** Tracking the tasks that occupy position 1 in some order and a later position in another:

| Task | as task 1 | at a later position | floor arm (no mechanism) |
|---|:---:|:---:|:---:|
| aquatic_mammals | **62.0** (order 0) | 71.0 (p8), 69.8 (p9) | 65.8 |
| insects | **77.4** (order 2) | 84.6 (p8), 85.6 (p20) | 81.6 |
| large_carnivores | **81.2** (order 3) | 83.4 (p4), 85.4 (p9) | 81.0 |

In all three cases the task scores **2–9 pts lower at position 1** than at a later position, and at position 1 it lands **at or below the mechanism-free floor** while comfortably beating it later. The mechanism is structural: the first task has no earlier frozen weights to pick from, so it gets only its own free weights and then pays the full 60 % compaction tax; later tasks pay the same tax but can pick through everything frozen before them, which more than compensates.

Three consequences:

1. **Cross-task reuse is load-bearing after all, and this quantifies it.** Picking is worth roughly 2–9 pts per task on this benchmark — the difference between a task that can reuse frozen weights and the same task that cannot. This is the direct evidence Section 9.12 item 2 previously asserted without measurement.
2. **It reconciles with the 50-task result (Section 9.8, finding 1b), where the floor beats masks.** Reuse gives a real per-task benefit, but the compaction tax is paid by every task including the first. On near-ceiling 2-class tasks there is no headroom for the reuse benefit to materialise, so only the tax shows; on harder 5-class tasks reuse pays for it. Mechanism value again tracks available headroom.
3. **It partly explains Section 9.10's +1.07 from accuracy-goal compaction, confirming threat item 11(b).** The gate fired on exactly one task of twenty — `aquatic_mammals`, which is both the weakest task in every run *and* first in the published order, where pruning cost it 18.3 train-accuracy points. That is precisely the first-task situation: no reuse to fall back on, full tax. Adaptive sparsity helps most where reuse is unavailable, so part of that +1.07 is a fix for a position artifact rather than a general improvement.

**Limits.** One run per shuffled order, and per-task accuracy is itself noisy (500 test images per task gives a binomial sd of ~2 pts at these accuracies), so each individual task delta is 2–3 sd. The evidence is that all three tasks move the same direction by more than that noise, which is much stronger than any one of them; a full 3-seeds x 3-orders design (9 runs) would separate order variance from seed variance properly and is the natural follow-up.

### 9.12 Where masks remain the right choice
...
2. **Cross-task weight reuse is real and load-bearing — now measured at 2–9 pts per task.** In the 50-task run, picking alone carried the last 38 tasks with the free pool pinned at 4.8 %; later tasks *function through* earlier tasks' frozen weights rather than around them. The order ablation (Section 9.11) quantifies the benefit directly: the same task scores 2–9 pts higher at a later position than at position 1, where nothing has been frozen yet to pick from — at position 1 it falls to or below the mechanism-free floor, and above it everywhere else. LoRA has no reuse at all: every task re-derives its adaptation from the pretrained initialization. Two caveats keep this honest: the benefit is measured on three tasks with one run per order, and it does not make masks the better mechanism overall — on the 50-task split the compaction tax paid by every task outweighs it and a no-reuse control wins (Section 9.8, finding 1b). Reuse is a real mechanism doing real work, not a reason the system wins.
...
4. **Structured growth with per-epoch serving cost** (Section 8.2): a task provably uses no unit added after it, so early tasks are served at their epoch's smaller FLOP count forever — a property adapter methods, which always run the full backbone, cannot offer. This does **not** contradict item 1: because growth appends *whole units*, the pre-growth network is a strict sub-network of the grown one, sharing one weight tensor set. There is still exactly **one stored artifact**; serving an early task means executing a slice of that single checkpoint (skipping units appended later), not fetching or versioning a second model. The cost is one execution configuration per growth epoch — a runtime dispatch concern, not a storage or versioning one.
...
### 9.13 Consolidated result narrative

| Experiment | Configuration | Retained acc | cAPF | Key takeaway |
|-----------|---------------|:---:|:---:|--------------|
| Ladder rung 0 | linear probe, frozen S@128 + per-task head | 69.51 % | — | Headline is not just pretrained features (−10.6 vs CPG) |
| Ladder rung 1 | floor-only control (per-task BN/bias/attn-bias, no adapter), 3 seeds | 77.97 ± 0.34 % | 3390 | **The per-task floor, not the mechanism, is the largest lever**: CPG adds +2.10, LoRA +4.24; ties the VGG scratch repro (p=0.08) at 32x fewer FLOPs |
| A1 reference | VGG16@1.5 CPG, official code | 78.61 % | 105 | Reproduction within 2.6 pts of paper (single seed) |
...
| Fairness control | VGG16-CPG, ImageNet init (1 seed) | 81.66 % | 109 | XL@128 above it on every seed, but +0.62 is p=0.073 — **parity**, not a win |
...
| Crossover 50-task | pair50, CPG (3 seeds) vs LoRA r2/r8, S@128 | 95.44 ± 0.02 / 96.82 / 96.99 % | — | No practical crossover: LoRA r=2 ties CPG's storage (30.7 MB) at +1.38 acc; per-task fp32 floor dominates both mechanisms |
| Floor arm, 50-task | pair50 floor-only control (no mechanism), 3 seeds | **95.71 ± 0.02 %** @ 19.2 MB | — | **Pareto-dominates CPG masks on both axes**: +0.27 acc (p=0.00009) for 37.6 % less storage. CPG's mechanism is slightly worse than not having it here; LoRA r=2 still buys +1.11 |
...
| Goal compaction | 20-task S@128, adaptive sparsity, seed 1 | 81.14 % | ~3533 | +1.07 over fixed 0.6 (single seed, no significance claim); gate fired on 1/20 tasks; halves the gap to LoRA r=8 |

**One-line claim:** CPG continual learning ported to the CascadedViT family forgets *exactly nothing* — verified to the bit in every run — while matching and exceeding the original paper's accuracy at 5–32x fewer inference FLOPs (parity, not a win, against the pretrained-VGG control); within the same protocol, per-task LoRA on the frozen backbone is the stronger mechanism on both accuracy and storage, a controlled comparison (with its negative results) that is itself a central finding.

**Consolidated claim (full):** Exact zero-forgetting continual learning — BWT +0.000 %, frozen-weight drift 0.00e+00, and old-task logits reproduced bit-for-bit (logit drift 0.00e+00) in every run — on the CascadedViT family matches and exceeds the original CPG paper's accuracy (82.3 ± 0.3 % over 3 seeds vs 81.2 % with CViT-XL at 128px input, every seed above) at 5x fewer inference FLOPs (t = 6.07, p = 0.026), and exceeds the VGG16-CPG reproduction at 32x fewer FLOPs (CViT-S @128, 80.1 ± 0.3 %; t = 10.1, p = 0.010). The advantage survives the fairness control as parity: with identical ImageNet pretraining given to VGG16-CPG (81.66 %, single seed), CViT-XL@128 is above it on every seed at 5x fewer FLOPs, though the +0.62 margin does not clear a 95 % interval (p = 0.073). A mechanism ladder on the identical frozen backbone calibrates all of this: a linear probe reaches 69.51 %, and a floor-only control that adds just the per-task BatchNorm/bias/attention-bias state both mechanisms must store anyway — no mask, no adapter — already reaches 77.97 ± 0.34 % (3 seeds), statistically tying the VGG16-CPG scratch reproduction at 32x fewer FLOPs; on top of that floor CPG masks add +2.10 (p = 0.0014) and LoRA r=8 adds +4.24 (p = 0.0001). The per-task floor is thus the largest single lever on this benchmark, the mechanism comparison is a comparison of refinements above it, and — strikingly — the adapters' advantage over the masks (+2.13) is the same size as the masks' entire advantage over no mechanism at all (+2.10). Extending the same control to the 50-task split sharpens this further: there the floor alone reaches 95.71 ± 0.02 % at 19.2 MB against CPG masks' 95.44 ± 0.02 % at 30.8 MB (3 seeds each, Welch t = 17.1, p = 0.00009), so on that benchmark the mask mechanism is Pareto-dominated *on both axes* by storing per-task normalization state and nothing else, while per-task LoRA still buys a real +1.11. Within the same protocol, however, the mechanism comparison is won by per-task low-rank adaptation: LoRA on the frozen backbone exceeds CPG masks at matched storage (82.20 ± 0.26 vs 80.07 ± 0.25 on S; 84.53 vs 82.28 on XL) at the cost of strictly linear per-task storage growth with no weight sharing — and the storage-crossover this trade-off promises to masks never materializes: over a 50-task sequence LoRA r=2 matches CPG's measured total storage (30.7 MB) while retaining +1.4 pts more accuracy, because a mechanism-independent per-task fp32 floor (BN / attention-bias / head state) dominates both mechanisms' growth. Compressing that floor does not reopen the question: sharing BN statistics collapses new-task learning (a negative result showing BN statistics are genuinely task-specific state), and fp16 per-task stores — applied symmetrically, guarantee intact — favor adapters (18.8 vs 24.6 MB at 50 tasks, accuracy edge unchanged), because CPG's incompressible residue is its own binarized piggymask. Pushing to the final rung with BitDelta-style 1-bit sign+scale stores compresses the *trained* per-task floor essentially for free on both arms (LoRA r=2: 96.54 % at 16.0 MB; CPG: 94.99 % at 21.8 MB; drift instruments exactly zero throughout) while isolating the floor's true incompressible core as the fp16 BN statistics; the same treatment applied to the LoRA factors themselves costs 20.2 pts — rank-2 adapters are already the compressed representation, so the two compressions do not stack — leaving the verdict intact and symmetric at every precision from fp32 to 1 bit. Restoring CPG's accuracy-goal compaction recovers half of the adapter accuracy edge (+1.07 pts, 81.14 % on the 20-task headline, seed 1). The controlled masks-vs-adapters comparison, including these negative results and the floor decomposition, is a central finding of this study. Finally, CPG's growing stage — unsound on chunk-routed attention under the published width-multiplier form (old-task logit drift 0.978) — is restored exactly by unit-granular growth (whole heads and FFN chunks at fixed per-unit dim): a growth event at task 11 of 20 leaves every earlier task bit-preserved in all three seeds (BWT +0.000 %, frozen-weight drift 0.00e+00, accuracy rows exactly flat; retained 71.33 ± 0.50 %).

---

## 10. Threats to validity and limitations

0. **Mechanism contribution is smaller than the headline suggests.** The floor-only control (Section 9.7, rung 1) retains 77.97 ± 0.34 % over three seeds with no adaptation mechanism at all, so CPG masks contribute **+2.10** pts and LoRA r=8 contributes **+4.24** pts over a baseline that any exact-guarantee method on this backbone gets essentially for free — and the floor alone ties the VGG16-CPG scratch reproduction (p = 0.080) at 32x fewer FLOPs. This does not affect the zero-forgetting results (which are exact and mechanism-specific) or the storage analysis, but it bounds how much of the accuracy story belongs to continual learning as opposed to per-task normalization, and it is the single most important caveat on the headline numbers. All three 20-task ladder steps are significant under Welch's t with three seeds per arm; only rung 0 (the probe) remains single-seed.

   **On the 50-task split the bound goes negative for CPG, and this is now replicated.** The same control retains 95.71 ± 0.02 % against CPG's 95.44 ± 0.02 % at 37.6 % less storage, over three seeds per arm (Welch t = 17.06, p = 0.00009, non-overlapping intervals; Section 9.8, finding 1b). On that benchmark the mask mechanism is Pareto-dominated on both axes by storing per-task normalization state alone. The margin is small in absolute terms, and it is specific to a near-ceiling 2-class regime — but it is a measurement, not an artifact of one run, and it is fatal to a mechanism whose premise is that it earns its complexity. The remaining single-seed arms in that study are LoRA r=2 and r=8.

1. **Seed coverage is uneven.** The two headline points carry three seeds each (S@128: 80.07 ± 0.25 %; XL@128: 82.28 ± 0.31 %; every seed above its reference and exactly zero forgetting in all six runs), but the 32px family sweep, the width sweep, the 50-task crossover study (Section 9.8), the floor-compression and 1-bit-store runs (9.9), the mechanism-ladder rungs 0–1 (9.7), and the adaptive-sparsity result (9.10, +1.07 vs a 3-seed baseline) remain single-seed. Per the resolution floor of Section 7.1, the width-sweep 1.5-vs-2.0 difference (0.25) and the M-vs-L difference (0.07) are reported as ties, not as findings, and no conclusion in this report rests on a gap below 0.6 pts. The crossover study's storage curves are deterministic given the recipe; its accuracy gap (+1.36) exceeds the headline runs' seed spread (±0.25) but has not been replicated. Its 2-class tasks are also easy (~97 % ceiling), which compresses accuracy gaps; the storage conclusion is unaffected.
...
4. **Pretraining asymmetry — controlled, but the control is single-seed.** The family results use ImageNet-pretrained CViT backbones while the original VGG reference trains from scratch (per the original protocol). This is controlled directly: an ImageNet-pretrained VGG16-CPG run (identical official pipeline) reaches 81.66 %, and CViT-XL@128 is above it on every seed at 5x fewer FLOPs (Section 9.6, finding 6). **The margin is +0.62 and does not clear a 95 % interval (p = 0.073), and the reference is one run with no error bar**, so this supports parity at 5x fewer FLOPs rather than a win — see item 10 below. The from-scratch CViT numbers (70.5–70.8 %) remain reported so the scratch-vs-scratch comparison is also available.
...
9. **Growing — resolved at unit granularity, with scope caveats**: unit-granular growth (Section 8.2) restores CPG's third stage exactly on chunk-routed ViTs, verified at bit level across a mid-stream growth event in three seeded runs (BWT +0.000 %, drift 0.00e+00 in every seed). Remaining limits: the growth quantum is the lcm of the per-unit dims (ed0/2 per stage on S; the nh=3 stages of M/XL force a full-width quantum), and the accuracy upside at 20 tasks is small (71.33 ± 0.50 % over 3 seeds, between the single-seed static 1.0/1.5 rows — consistent with capacity not binding) — longer task streams, where capacity does bind, are where the mechanism should matter. **No experiment in this report constructs a capacity-bound regime**, so the growing stage is verified correct and shown unnecessary at every horizon tested; an accuracy motivation for it remains future work (candidates: a narrower backbone, a stream beyond 50 tasks, or the lower per-task sparsities `--adaptive-sparsity` selects, all of which consume the free pool faster).

10. **The VGG16-CPG references are single runs.** Both the scratch reproduction (78.61 %) and the ImageNet-pretrained fairness control (81.66 %) are one seed each, with no measured variance, while the CViT points they are compared against carry three seeds. The two comparisons that clear a 95 % interval (S@128 vs 78.61, p = 0.010; XL@128 vs the paper's 81.2, p = 0.026) treat the reference as a fixed constant, which is optimistic — a reference error bar of the same order as CViT's (±0.25–0.31) would widen both. The XL-vs-pretrained-VGG comparison (+0.62, p = 0.073) is the one materially affected: it is reported as parity, not as a win. Resolving it requires seeds 2–3 of the pretrained VGG arm, the most compute-expensive outstanding item in this report (VGG16@1.5 at 0.7467 GFLOPs x 20 tasks x 2 runs).

11. **Task order — measured, with a first-task penalty found; the headline order is mildly favourable.** Every headline run uses one fixed task order, so CPG's order dependence was originally unmeasured. It has now been ablated (Section 9.11): CPG's spread across three orders is 1.00 pt against 0.50 pt across three seeds at fixed order, and the per-task breakdown identifies the cause — a task placed **first** scores 2–9 pts below its own accuracy at a later position, and at or below the mechanism-free floor, because it has no earlier frozen weights to pick from yet pays the full 60 % compaction tax. Residual limitations: one run per shuffled order, so order effect and run noise are not cleanly separated (a 3-seeds x 3-orders design would do this); only three tasks have been observed in position 1; and per-task accuracy carries ~2 pts of binomial noise, so the evidence is the consistent direction across three tasks rather than any single delta. Two downstream consequences are now on the record rather than speculative: cross-task reuse is load-bearing and worth 2–9 pts per task (Section 9.12, item 2), and part of Section 9.10's +1.07 from accuracy-goal compaction is a fix for the first-task penalty, since the gate fired on exactly the task sitting in position 1. The published order is therefore not neutral — it places `aquatic_mammals`, the weakest task in every run, in the most penalised slot — but it is not adversarial either: order-seed 3 scored above it and order-seed 2 below.
...
# mechanism ladder, rung 0: linear probe (frozen backbone + per-task head)
python probe_cvit.py --tasks 20 --variant S --img-size 128 --epochs 50 \
    --results-file cvit_probe_20task_S_128.txt

# mechanism ladder, rung 1: floor-only control (per-task BN/bias/attn-bias, no adapter)
python train_lora_cvit.py --tasks 20 --epochs 29 --variant S --img-size 128 \
    --rank 0 --results-file cvit_floor_20task_S_128.txt

# task-order ablation (Section 10, item 11); --order-seed 0 = published order
python train_cpg_cvit.py --tasks 20 --variant S --img-size 128 --pretrained \
    --order-seed 2 --results-file cvit_cpg_20task_S_128_order2.txt
python train_lora_cvit.py --tasks 20 --epochs 29 --variant S --img-size 128 \
    --rank 8 --order-seed 2 --results-file cvit_lora_20task_S_128_order2.txt   # null control

# 50-task storage-crossover study (CPG + LoRA r8 + LoRA r2, sequential) + figure
bash run_crossover50.sh

# 50-task floor arm (no mechanism) -- the reference line for the crossover figure
python train_lora_cvit.py --split pair50 --tasks 50 --epochs 29 --variant S \
    --img-size 128 --rank 0 --results-file cvit_lora_pair50_S_128_r0_seed1.txt
...
| `cvit_cifar/probe_cvit.py`, `cvit_probe_20task_S_128.txt` | Mechanism ladder rung 0: linear probe (Section 9.7, 69.51 %) |
| `cvit_cifar/cvit_floor_20task_S_128{,_seed2,_seed3}.txt` | Mechanism ladder rung 1: floor-only control, 3 seeds (`train_lora_cvit.py --rank 0`) |
| `cvit_cifar/cvit_lora_pair50_S_128_r0_seed{1,2,3}.txt` | 50-task floor arm, 3 seeds (Section 9.8, finding 1b) — Pareto-dominates CPG masks |
| `cvit_cifar/cvit_cpg_pair50_S_128_seed{1,2,3}.txt`, `run_crossover50_seeds.sh` | 50-task CPG arm, 3 seeds + the seed-replication driver |
