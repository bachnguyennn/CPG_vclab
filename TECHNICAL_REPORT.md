...
**Run-to-run noise.** Two runs of the *identical* from-scratch configuration (CViT-S w1.0, 32px) gave 70.53 % and 70.76 % — a spread of **0.23 pts** with no intended difference between them (Section 9.12). Three-seed points show standard deviations of 0.25–0.50. We therefore adopt a **resolution floor of ~0.6 pts**: differences smaller than this are reported as *ties / unresolved* and no finding in this report rests on one. Specifically unresolved by this criterion, and labelled as such where they appear:
...
**The ladder is benchmark-dependent, and the dependence is the finding.** Re-running rung 1 over the 50-task 2-class split (Section 9.8, finding 1b) gives 95.70 % against CPG's 95.46 % and LoRA r=2's 96.82 % — so on that easier benchmark the masks' +2.10 advantage measured here disappears entirely while the adapters' shrinks to +1.12. Mechanism value is a function of how far the frozen backbone plus per-task recalibration already gets you: near a ~97 % ceiling, nothing is left for a mechanism to recover.
...
|---|:---:|:---:|:---:|:---:|:---:|
| **Floor only (no mechanism)** | **95.70 %** | **19.18 MB** | **0.249** | +0.000 % | 0.00e+00 / 0.00e+00 |
...
1b. **On this split, the mechanism-free floor Pareto-dominates CPG masks.** Running the rung-1 control (Section 9.7) over the same 50 tasks retains **95.70 %** at **19.18 MB** — statistically the same accuracy as CPG masks (95.46 %; the +0.24 gap is inside the 0.23-pt run-to-run noise of Section 7.1, and both arms are single-seed) for **37.6 % less storage**, at roughly half the late-sequence slope (0.249 vs 0.459 MB/task). **CPG's mechanism buys nothing measurable here and costs 11.6 MB.** By contrast LoRA r=2 does buy something real — +1.12 over the floor — for its extra 11.5 MB. The mechanism comparison of Section 9.7 therefore inverts with task difficulty: on the 20-task 5-class split, masks were worth +2.10 over the floor; on the 50-task 2-class split, where per-task accuracy sits near a ~97 % ceiling, they are worth nothing and adapters are worth about half what they were. The natural reading is that per-task normalization state alone saturates easy tasks, and a weight-adaptation mechanism only earns its storage when the tasks are hard enough that the backbone's frozen features plus recalibration fall short. This does not weaken the storage conclusion of finding 1 — it strengthens it, by adding a cheaper arm that CPG also fails to beat.
2. **Why: a mechanism-independent per-task fp32 floor dominates both — on storage and, as Section 9.7 shows, on accuracy.** Any exact-zero-forgetting method on this backbone must store per-task BN statistics, attention-bias tables, and a head (~0.24 MB/task). That same floor, trained on its own with no mask and no adapter, already retains 77.97 ± 0.34 % of the 20-task benchmark (Section 9.7, rung 1) and 95.70 % of this one (finding 1b) — so it is not merely a storage overhead the mechanisms carry, it is where most, and here all, of the accuracy comes from. CPG's binarized piggymask adds ~0.21 MB/task; LoRA r=2's adapters add ~0.24 MB/task. The *mechanism* is a minority share of the per-task bill on both sides, so the storage lines run near-parallel — the crossover the mask design promises is neutralized by overhead the masks cannot remove. Design implication: to beat adapters on storage, shrink the fp32 floor (shared or quantized per-task BN/head state), not the mask.
...
| Floor arm, 50-task | pair50 floor-only control (no mechanism) | **95.70 %** @ 19.2 MB | — | **Pareto-dominates CPG masks**: same accuracy (within noise) for 37.6 % less storage. CPG's mechanism buys nothing on this split; LoRA r=2 still buys +1.12 |
...
0. **Mechanism contribution is smaller than the headline suggests.** The floor-only control (Section 9.7, rung 1) retains 77.97 ± 0.34 % over three seeds with no adaptation mechanism at all, so CPG masks contribute **+2.10** pts and LoRA r=8 contributes **+4.24** pts over a baseline that any exact-guarantee method on this backbone gets essentially for free — and the floor alone ties the VGG16-CPG scratch reproduction (p = 0.080) at 32x fewer FLOPs. This does not affect the zero-forgetting results (which are exact and mechanism-specific) or the storage analysis, but it bounds how much of the accuracy story belongs to continual learning as opposed to per-task normalization, and it is the single most important caveat on the headline numbers. All three 20-task ladder steps are significant under Welch's t with three seeds per arm; only rung 0 (the probe) remains single-seed.
   ...
   **On the 50-task split the bound tightens to zero for CPG.** The same control retains 95.70 % against CPG's 95.46 % at 37.6 % less storage (Section 9.8, finding 1b), so on that benchmark the mask mechanism contributes nothing measurable and is Pareto-dominated by storing per-task normalization state alone. Both 50-task arms are single-seed and the accuracy gap is inside the run-to-run noise, so the honest statement is a *tie on accuracy at substantially lower storage*, not a win — but a tie is already fatal to a mechanism whose entire premise is that it earns its complexity. Replication at 2–3 seeds is the highest-value outstanding run in this report.
...
| `cvit_cifar/cvit_lora_pair50_S_128_r0_seed1.txt` | 50-task floor arm (Section 9.8, finding 1b) — Pareto-dominates CPG masks |
