#!/bin/bash
# SMOKE TEST: identical compact->pick->grow logic as CPG_cifar100_scratch_mul_1.5.sh
# but limited to 2 tasks with tiny epoch counts, just to validate the pipeline.
# Not for real numbers. Run from the repo root with the `cpg` conda env active.
...
num_classes=5
batch_size=32
total_num_tasks=2                 # was 20

export PYTORCH_ENABLE_MPS_FALLBACK=1

for task_id in `seq 1 2`; do

    # Training the network on current task (Pick + Grow)
    state=2
    while [ $state -eq 2 ]; do
        if [ "$task_id" != "1" ]
        then
            python CPG_cifar100_main_normal.py \
                ...
                --lr $lr --lr_mask $lr_mask --batch_size $batch_size --weight_decay 4e-5 \
                --workers 4 \
                ...
                --epochs $finetune_epochs --mode finetune \
        ...
        then
            echo "Provide baseline_cifar100_acc.txt as criterion for network capacity."
            exit 0
        fi
    done

    nrof_epoch_for_each_prune=2    # was 20
    nrof_epoch=$nrof_epoch_for_each_prune
    start_sparsity=0.0
    end_sparsity=0.1

    # Compacting: gradual pruning
    if [ $state -ne 5 ]
    then
        echo "state=$state -> begin gradual pruning"
        python CPG_cifar100_main_normal.py \
            --arch $arch \
            --dataset ${dataset[task_id]} --num_classes $num_classes \
            --lr $gradual_prune_lr --lr_mask 0.0 --batch_size $batch_size --weight_decay 4e-5 \
            --workers 4 \
            ...
            --epochs $nrof_epoch --mode prune \
            --initial_sparsity=$start_sparsity --target_sparsity=$end_sparsity \
    ...
    # Pick the checkpoint with the best sparsity meeting the accuracy goal
