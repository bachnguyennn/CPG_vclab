...
def main():
    ...
    logging.info('using device: %s', utils.DEVICE)
    args.cuda = (utils.DEVICE.type == 'cuda')
    ...
    if not masks:
        for name, module in model.named_modules():
            if isinstance(module, nl.SharableConv2d) or isinstance(module, nl.SharableLinear):
                ...
                if True:  # move mask to the weight's device (cuda/mps/cpu)
                    mask = mask.to(module.weight.data.device)
                masks[name] = mask
    else:
